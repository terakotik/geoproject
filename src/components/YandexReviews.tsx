...
const ICONS = {
    ...
    left: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="hsl(var(--foreground))" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>`,
    right: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="hsl(var(--foreground))" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>`
};
...
export function YandexReviews() {
    ...
    useEffect(() => {
        ...
        // We need to ensure the DOM is ready for the slider to initialize
        ...
        return () => window.removeEventListener('load', initSlider);
    }, []);
    ...
    return (
        <>
            <style jsx global>{`
                ...
                .ya-nav-btn { position: absolute; top: 50%; transform: translateY(-50%); width: 44px; height: 44px; background: hsl(var(--card)); border: 1px solid hsl(var(--border)); border-radius: 50%; box-shadow: 0 4px 10px rgba(0,0,0,0.1); display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 10; transition: opacity 0.3s, transform 0.2s; }
                ...
                .ya-prev { left: -15px; }
                .ya-next { right: -15px; }
                @media (max-width: 520px) { .ya-prev { left: 5px; } .ya-next { right: 5px; } .ya-container { padding-left: 15px; padding-right: 15px; } }
            `}</style>
